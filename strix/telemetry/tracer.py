...
class Tracer:
    ...
    def _append_event(self, kind: str, **data: Any) -> None:
        """Append one structured event to ``events.jsonl`` in the run dir.

        This is the live feed the web console tails to mirror what the CLI's
        Textual UI renders (thinking messages, tool calls, agent tree). Purely
        additive telemetry — best-effort and never allowed to disrupt a scan.
        """
        try:
            run_dir = self.get_run_dir()
            event = {"ts": datetime.now(UTC).isoformat(), "kind": kind, **data}
            with (run_dir / "events.jsonl").open("a", encoding="utf-8") as f:
                f.write(json.dumps(event, ensure_ascii=False, default=str) + "\n")
        except (OSError, TypeError, ValueError):
            logger.debug("Failed to append %s event", kind, exc_info=True)
...
"""
    ...
    def log_agent_creation(
        ...
        self._append_event(
            "agent_created", agent_id=agent_id, name=name, task=task, parent_id=parent_id
        )
    ...
    def log_chat_message(
        ...
        if emitted:
            self._append_event(
                ...
                role=role,
                agent_id=agent_id,
                content=emitted[:8000],
                is_thinking=bool(thinking and not body),
                interrupted=bool((metadata or {}).get("interrupted")),
            )
        return message_id
    ...
    def update_tool_execution(
        ...
        if execution_id in self.tool_executions:
            ex = self.tool_executions[execution_id]
            ex["status"] = status
            ex["result"] = result
            ex["completed_at"] = datetime.now(UTC).isoformat()
    ...
    def update_agent_status(
        ...
        if agent_id in self.agents:
            ...
            self._append_event(
                "agent_status", agent_id=agent_id, status=status, error_message=error_message
            )
